...
size_t explore_graph(auto &node, std::stack<std::string> &path, auto path_count,
                     auto &total_paths) {
    ...
    for (auto &next_node : node->next_nodes) {
        ...
        if (!(next_node->can_be_visited_twice) && !(next_node->is_end) &&
            !(next_node->is_big) && next_node->visit_counter > 0) {
            continue;
        }
        if ((next_node->can_be_visited_twice) && !(next_node->is_end) &&
            next_node->visit_counter > 1) {
            continue;
        }

        path_count = explore_graph(next_node, path, path_count, total_paths);
    }
...
struct Node {
    Node(std::string _id)
        : is_big(false), is_start(false), is_end(false),
          can_be_visited_twice(false), visit_counter(0), id(_id){};

    bool is_big;
    bool is_start;
    bool is_end;
    bool can_be_visited_twice;
    int visit_counter;
    std::string id;
...
int main() {
    ...
    for (auto &node : nodes) {
        if (node->is_big || node->is_start || node->is_end) {
            continue;
        }
        // if (!(node->is_end) && !(node->is_start) && !(node->is_big)) { // }
        std::cout << "Setting small node " << node->id << " to be visited twice"
                  << std::endl;

        node->can_be_visited_twice = true;
        ...
        auto path_count = explore_graph(start_node, path, 0, all_paths);

        std::cout << "Number of pathes " << path_count << std::endl;

        // Revert
        node->can_be_visited_twice = false;
    }

    std::cout << "Coucou, la réponse est " << all_paths.size();
