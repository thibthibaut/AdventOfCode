...
void find_bassin(auto low_point, const auto& height_map, auto& bassin_map, int label, int previous_value);
...
int main() {
  ...
  std::transform(counts.begin(), counts.end(), std::back_inserter(counts_vec), [](const auto val){return val.second;});
...
void find_bassin(auto low_point, const auto& height_map, auto& bassin_map, int label, int previous_value){
  ...
  if( low_point_value <= previous_value) return;
  //if( low_point_value != previous_value+1) return;
  if(low_point_value == 9) return;
  ...
  if( low_point.second > 0){
    auto new_low_point = std::make_pair(low_point.first, low_point.second - 1);
    find_bassin( new_low_point, height_map, bassin_map, label, low_point_value);
  }
  // Check right:
  ...
  if( low_point.first > 0) {
    auto new_low_point = std::make_pair(low_point.first - 1, low_point.second);
    find_bassin( new_low_point, height_map, bassin_map, label, low_point_value);
  }
  // Check down:
