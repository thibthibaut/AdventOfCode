...
int main() {
    ...
    for(auto pivot = *min; pivot < *max+1; pivot++){
      ...
      for(const auto elem : crabs){
        sum_of_distances += std::abs(elem - pivot);
      }
    ...
    auto min_val = std::min_element(distance_map.begin(), distance_map.end(), [](const auto& lhs, const auto& rhs){ return lhs.second < rhs.second;});

    std::cout << "Minimum element is " << std::distance(std::begin(distance_map), min_val) 
      << " With " << min_val->second << " Fuel " << std::endl;
